...
This document describes the architecture and design of the Sandbag system.

== Overview

Sandbag is a Clojure web service built on three core technologies:
...
Routes are defined in `sandbag.service.routes` using Pedestal's terse routing syntax:
...
|`body-params`
|Parse request body (JSON, EDN, form data)

|`url-decode-path-params`
|Decode URL-encoded path parameters

|`data-body`
|Set up response body serialization

|`accept-content`
|Content negotiation (JSON, EDN, Transit)

|`parsed-params`
|Merge and parse all parameters

|`validated-params`
|Validate parameters against schema

|`log-params`
|Log request parameters (development)

|`log-response`
|Log response details (development)
|===
...
The `sandbag.db.datatype` namespace provides the metamodel API:
...
|`all-classes`
|List all class idents

|`all-properties`
|List all property idents
...
|`slots-of`
|Get all effective slots (inherited + direct)
...
{:http-port 8080
 :nrepl-port 28888
 :db-uri "datomic:dev://localhost:4334/sandbag"
 :required-schema [:meta :literal :ref :fn :any :user :twit]}
----

Accessed via `sandbag.util.edn`:
...
1. Define handler in `src/sandbag/api/`
2. Add validator in the handler file
3. Add route in `src/sandbag/service/routes.clj`
