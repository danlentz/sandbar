...
81 verbs across 22 axes — **existence-eager, detail-lazy**.  Rank verbs for a task with `sandbar.tools.search <intent>`; get the full card + composition edges (prerequisites / combines-with) with `sandbar.tools.describe <verb>`.  Safety: `[safe]` read-only · `[idem]` idempotent write · `[unsafe]` mutating.
