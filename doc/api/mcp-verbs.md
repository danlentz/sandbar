...
**Args** (`*` = required):
- `allow-unknown-keys` (boolean) — Opt out of strict front matter: carry unknown keys in the front-matter carrier instead of refusing them (D6, 2026-09-19).  Default false.
...
- `validation-mode` (string) — Shape validation: 'audit' (default) commits and returns the shape report; 'strict' validates against the class's shapes on a SPECULATIVE database BEFORE the transaction and refuses a violation with nothing committed, enqueued or notified (D6, 2026-09-19); 'disabled' skips the shapes.
...
**Args** (`*` = required):
- `exclude` (array) — Rel-paths of units to walk and fingerprint but neither plan nor transact — deferred rows kept with both versions held (the maintenance import, D7 2026-09-20).
- `expect-basis` (number) — Pin a persist to the :basis a dry run reported; refused if the database moved since (preview again).
- `expect-source-hashes` (object) — The dry run's :sources map ({rel-path sha256}); like expect-sources but the refusal names the files that changed, appeared or disappeared (D7-R5).
- `expect-sources` (string) — Pin a persist to the :sources-sha256 token a dry run reported: refused before any transaction if any walked file changed, appeared or disappeared since (D7-R5).
...
- `mode` (string) — 'replace' (default): a file the substrate already holds replaces its source-owned representation in one transaction; 'additive': the pre-D7 behaviour, assertions only (for ingestion from independent sources).
...
- `rank-by` (string) — Stage 29: re-rank axis — ':degree' / ':backlink-density' / ':recency' / ':freshness'; or ':relevance' to opt out of the default status- and recency-aware ordering (D4c) for pure BM25F order
