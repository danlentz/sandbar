(ns sandbar.security.query
  "Validate caller-supplied Datalog clauses and read-plane namespaces.

   Data-only EDN parsing prevents reader evaluation, but a query can still
   invoke a function named in a predicate clause. `sanitize-where` therefore
   admits only curated operator vars, supported query built-ins, or registered
   rules. Unsupported syntax, nested logic, operators, and namespaces fail
   loudly; no rejected filter is silently dropped.

   `safe-query-op-allowlist` and `safe-path-test-registry` define the reviewed
   operator vocabulary for query heads and unary path tests. Each surface
   further restricts its permitted shape. Resolution checks existing vars
   without loading arbitrary namespaces. Higher-order `apply` is excluded
   because it can hide a forbidden target; regex operators are excluded
   because unbounded matching work needs a separate execution budget.

   The namespace firewall admits the memory model and metamodel while
   excluding credential and runtime instance data. Memory compartment
   clearance is a separate decision in sandbar.security.visibility.
...
(def safe-path-test-registry
  "Map allowed path :TEST predicate names to fully qualified unary predicate
   symbols. The path compiler initializes its registry from this constant and
   validates registration and compilation against the shared operator policy.
   Keep changes here subject to the same review as query-head operators."
...
(def safe-operator-vocabulary
  "The union of resolved vars from the query-head allowlist and path-test
   registry. This is the shared vocabulary for operator resolution; membership
   alone does not grant every surface permission to invoke an operator, since
   each surface also restricts its accepted predicates and argument shapes."
...
(def registered-query-rules
  "Registered Datalog rule names expanded against the substrate rulebase,
   rather than resolved as classpath functions. The base queries normally
   supply these rules themselves. Definitions live in sandbar.db.datatype."
...
(defn read-plane-entity-visible?
  "Apply the namespace firewall to an entity's class, not its own ident.
   Class and property definitions remain introspectable as metamodel entities
   even when they describe a restricted namespace; instances of that namespace
   remain hidden. This distinguishes schema shape from credential values.

   Accept raw Datomic entities and projected maps. A class EntityMap supports
   keyword lookup even though map? is false. An absent class is allowed here;
   memory compartment clearance is handled separately."
  [entity]
